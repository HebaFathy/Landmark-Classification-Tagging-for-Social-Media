...
def get_data_loaders(
    ...
    """
    print("hello", multiprocessing.cpu_count())
    if num_workers == -1:
        num_workers = multiprocessing.cpu_count() # Use all GPUs
        print(num_workers)
    ...
    train_data = datasets.ImageFolder(
        base_path / "train",
        data_transforms["train"] #train transform
    )
    # The validation dataset is a split from the train_one_epoch dataset, so we read
    # from the same folder, but we apply the transforms for validation
    valid_data = datasets.ImageFolder(
        base_path / "train",
        data_transforms["valid"] #valid transform
    )
    ...
    test_data = datasets.ImageFolder(
        base_path / "test",
        data_transforms["test"] #test transform
    )
    ...
    data_loaders["test"] = torch.utils.data.DataLoader(
        test_data, #test_data
        batch_size=batch_size,
        sampler=test_sampler, #test_sampler
        num_workers=num_workers,
        shuffle=False
    )
